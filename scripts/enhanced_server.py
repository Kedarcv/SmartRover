...
Handles vehicle control, waypoint navigation, and mining operations
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/smartrover/server.log'),
...
vehicle_thread = None
mining_database = '/var/lib/smartrover/mining_data.db'
...
# Initialize database
db = MiningDatabase(mining_database)
...
def control_vehicle():
    ...
    try:
        ...
        if command == 'emergency_stop':
            ...
            log_system_event('CRITICAL', 'Emergency stop activated via dashboard')
            return jsonify({
                'success': True,
            ...
            log_system_event('INFO', 'Mining operation started via dashboard')
            return jsonify({
                'success': True,
            ...
            log_system_event('INFO', 'Mining operation stopped via dashboard')
            return jsonify({
                'success': True,
            ...
            log_system_event('INFO', 'Return to dock initiated via dashboard')
            return jsonify({
                'success': True,
...
def get_waypoints():
    """Get all waypoints"""
    try:
        conn = sqlite3.connect(mining_database)
        cursor = conn.cursor()
...
def get_map_data():
    """Get current map data with waypoints"""
    try:
        if vehicle_controller:
            map_data = vehicle_controller.slam_mapper.export_map_data()
            
            # Add waypoints to map data
            conn = sqlite3.connect(mining_database)
            cursor = conn.cursor()
            cursor.execute('SELECT id, name, x, y, type, status FROM waypoints')
    ...
    try:
        return jsonify({
            ...
            'data': {
                ...
                'bluetooth_clients': 0,  # Placeholder
                'timestamp': time.time()
            }
        })
    except Exception as e:
...
def start_vehicle_thread():
    ...
    def run_vehicle():
        ...
        try:
            vehicle_controller = VehicleController(mining_database)
...
def health_check():
    ...
    return jsonify({
        ...
        'services': {
            ...
            'sensors': vehicle_controller.sensor_array is not None if vehicle_controller else False
        }
    })
...
if __name__ == '__main__':
    logger.info("Starting SmartRover Mining Vehicle Server v2.0...")
